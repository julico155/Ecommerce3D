...
<body>
    ...
    @section('cliente')
        <div class="container mx-auto mt-4">
            <div class="text-center">
                <a href="{{ route('welcome') }}">
                    <h1 class="text-4xl text-black font-bold mb-6">REFRACGAS</h1>
                </a>
            </div>
            <div
                class="bg-gray-200 justify-center items-center shadow-xl rounded-2xl p-4 lg:p-8 mb-8 grid grid-cols-[3fr,2fr]">
                ...
                <div class="space-y-2 mt-4">
                    <h2 class="text-2xl font-semibold text-gray-800">{{ $p->nombre }}</h2>
                    ...
                    @auth
                        <form action="{{ route('carrito.update', $p->id) }}" method="post">
                            ...
                            <div class="flex items-center">
                                <input type="number" name="cantidad" placeholder="Cantidad" required min="1"
                                    max="{{ $p->stock }}" class="border border-gray-300 px-4 py-2 rounded-l-md w-32">
                                <button type="submit"
                                    class="bg-green-500 text-white px-4 py-2 rounded-r-md hover:bg-green-600 transition duration-300">Agregar
                                    al Carrito</button>
                            </div>
                        </form>
                        <!-- Botón para abrir el modal, con ruta del modelo 3D -->
                        @if ($p->es_3d)
                            <h2 class="text-lg font-semibold text-gray-800 mt-4">Modelo 3D</h2>
                            <button type="button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                                data-toggle="modal" data-target="#model3DModal">
                                Visualizar Modelo 3D
                            </button>
                            <p class="text-sm text-gray-600"><strong>Descripción:</strong> {{ $p->descripcion_3d }}</p>
                            <p class="text-sm text-gray-600"><strong>Precio:</strong> BOB. {{ $p->precio_3d }}</p>
                            <div class="flex gap-2">
                                <p class="text-sm text-gray-600 text-bold"><strong>Formatos:</strong></p>

                                @if ($p->es_formato_obj)
                                    <p class="text-sm text-gray-600">.Obj</p>
                                @endif
                                @if ($p->es_formato_gltf)
                                    <p class="text-sm text-gray-600">.gltF</p>
                                @endif
                                @if ($p->es_formato_fbx)
                                    <p class="text-sm text-gray-600">.fbx</p>
                                @endif
                                @if ($p->es_formato_stl)
                                    <p class="text-sm text-gray-600">.stl</p>
                                @endif
                            </div>
                        ...
                        @endif
                    @else
                        <p class="text-red-600 bg-red-100 border border-red-600 rounded-md px-4 py-2">Inicia sesión para
                            poder comprar</p>
                    @endauth
                </div>
        ...
        <div class="modal fade" id="model3DModal" tabindex="-1" role="dialog" aria-labelledby="model3DModalLabel"
            aria-hidden="true">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            $('#model3DModal').on('shown.bs.modal', function() {
                ...
                // Crear la cámara
                const camera = new BABYLON.ArcRotateCamera("camera", Math.PI / 2, Math.PI / 2, 2,
